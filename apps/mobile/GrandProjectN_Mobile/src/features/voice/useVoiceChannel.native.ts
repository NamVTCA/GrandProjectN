...
export function useVoiceChannel(roomId: string) {
  ...
  const getBlankVideoTrack = useCallback((): MediaStreamTrack => {
    // React Native không có canvas, sử dụng black video track thay thế
    // Trong thực tế, bạn cần tạo một black video track bằng cách khác
    return {} as MediaStreamTrack;
  }, []);

  const ensureCurrentVideoTrack = useCallback(() => {
    return getBlankVideoTrack();
  }, [getBlankVideoTrack]);
  ...
  const startShare = useCallback(() => {
    if (sharing) return;
    // Trong React Native, bạn cần sử dụng thư viện chuyên dụng để chia sẻ màn hình
    console.log("Screen sharing not implemented in React Native version");
