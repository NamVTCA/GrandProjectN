...
export class PaymentsService {
  ...
  constructor(
    ...
    @InjectModel(CoinPackage.name) private coinPackageModel: Model<CoinPackageDocument>,
    ...
    this.stripe = new Stripe(stripeSecretKey, { apiVersion: '2024-06-20' as any });
  }

  async createPaymentIntent(user: UserDocument, packageId: string): Promise<{ orderId: string; clientSecret: string }> {
    ...
    if (!paymentIntent.client_secret) {
        throw new InternalServerErrorException('Không thể tạo yêu cầu thanh toán từ Stripe.');
    }
  ...
  async fulfillOrder(orderId: string): Promise<{ message: string }> {
    const order = await this.orderModel.findById(orderId).populate('coinPackage');
    ...
    if (!updatedUser) {
      throw new InternalServerErrorException('Không thể cập nhật số dư của người dùng.');
    }
