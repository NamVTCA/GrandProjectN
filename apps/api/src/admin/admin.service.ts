...
import { User, UserDocument, GlobalRole, AccountStatus } from '../auth/schemas/user.schema';
...
export class AdminService {
    ...
    async updateUserRole(userId: string, role: GlobalRole): Promise<User> {
     ...
     // --- HÀM MỚI ---
    async suspendUser(userId: string, reason: string, durationInDays: number, admin: UserDocument): Promise<User> {
        ...
        user.warnings.push({ reason: `Tạm khóa: ${reason}`, date: new Date(), by: admin._id });

        return user.save();
    }

    async banUser(userId: string, reason: string, admin: UserDocument): Promise<User> {
        const user = await this.userModel.findById(userId);
        if (!user) throw new NotFoundException('Không tìm thấy người dùng.');
        ...
        user.warnings.push({ reason: `Khóa vĩnh viễn: ${reason}`, date: new Date(), by: admin._id });

        return user.save();
    }
    
    async warnUser(userId: string, reason: string, admin: UserDocument): Promise<User> {
        const user = await this.userModel.findById(userId);
        if (!user) throw new NotFoundException('Không tìm thấy người dùng.');
        ...
        const posts = await this.postModel
            .find({ moderationStatus: { $in: [ModerationStatus.PENDING, ModerationStatus.PROCESSING] } })
     ...
     */
    async updatePostStatus(postId: string, status: ModerationStatus.APPROVED | ModerationStatus.REJECTED): Promise<Post> {
        const post = await this.postModel.findByIdAndUpdate(postId, { moderationStatus: status }, { new: true });
     ...
     */
    async updateCommentStatus(commentId: string, status: ModerationStatus.APPROVED | ModerationStatus.REJECTED): Promise<Comment> {
        const comment = await this.commentModel.findByIdAndUpdate(commentId, { moderationStatus: status }, { new: true });
