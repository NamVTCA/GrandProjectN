...
import { ShopItem, ShopItemDocument, ItemType } from './schemas/shop-item.schema';
...
export class ShopService {
  constructor(
    ...
    private inventoryService: InventoryService, // Service quản lý kho đồ
  ) {}

  // Lấy danh sách vật phẩm (có thể lọc theo type)
  async listItems(types?: ItemType[]): Promise<ShopItem[]> {
    const filter = types?.length ? { type: { $in: types } } : {};
    return this.shopItemModel.find(filter).sort({ createdAt: -1 }).lean().exec();
  }
  ...
  async updateItem(id: string, dto: UpdateShopItemDto): Promise<ShopItem> {
    const updated = await this.shopItemModel.findByIdAndUpdate(id, dto, { new: true });
