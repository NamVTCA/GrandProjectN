...
// ✅ HÀM NÀY ĐÃ ĐƯỢC HOÀN THIỆN
  async sendRequest(sender: UserDocument, recipientId: string): Promise<FriendRequest> {
    if (sender._id.toString() === recipientId) {
      throw new BadRequestException('Bạn không thể tự kết bạn với chính mình.');
    }

    // Kiểm tra xem đã là bạn bè chưa
    const isAlreadyFriend = sender.friends.some(friendId => friendId.toString() === recipientId);
    if (isAlreadyFriend) {
  ...
  async respondToRequest(
    ...
    if (status === FriendRequestStatus.ACCEPTED) {
      ...
      if (!sender) throw new NotFoundException('Không tìm thấy người gửi yêu cầu.');
